    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            if (availableUsers.length > 0) {
                ...
                showNoUsersMessage(); // Show message when all users are busy
            }
        } else {
            showNoUsersMessage(); // Show message when no active users are available
        }
    } catch (error) {
...
function showNoUsersMessage() {
    // Find or create the container for messages
    ...
    message.textContent = "All users seem busy at the moment.";
    ...
    backButton.href = "https://ai.findaguest.online";
    backButton.textContent = "Try This Feature for fun";
    ...
    // Optionally center the message container on the page
...
function redirectToChatRoom() {
    alert("Found someone.. Start chat now");
